...
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
//Sparse Tables => for answering the range minimum queries but not helpful in case if there are updates also in the queries
//Tc = Brute => O[n*q]
//Tc = optimised => O[nlog(n) + Q];
//concept of dance table -> if we precompute the value for each subarray Tc = O[n^2 + Q];
//Sparse table in which we only precompute for powersof 2
//as each number can be expressed in the powers of 2
//for the IDEMPOTENT queries i.e. overlaping doesn't matter like min, max, gcd not for sum

//preprocessing:-
// compute for 1
// for len 2 dependent on 1
//for len  dependent on 2 and so on...

//Answering queries
//taking the just smaller power of two and and the queiey in O[1] 
int Query(int L, int R, vector<vector<int>>&v){
	int len = R-L+1;
	len = log2(len);
	return min(v[L][len], v[R-(1<<len)+1][len]);
}
void solve(){
	int n;
	cin >> n;
